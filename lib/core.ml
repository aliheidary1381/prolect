...
let _false: solution_set = Seq.empty
...
let rec sld_resolution (sub : substitution) (trm_q: term) (cls: clause) : solution_set = 
  ...
  ) with Invalid_argument(_) -> _false
...
and prove_conjunction (sub: substitution) (conj: conjunction) : solution_set =
  ...
  conj |>
    fold (fun (acc: solution_set) (term: term) -> acc |>
...
let rec print_sub_list (ls': solution_set) = match Seq.uncons ls' with
  | None -> ()
